...
- [x] 使用者可建立、編輯、刪除私人旅程。
- [x] 旅程可保存國家、城市、日期、標題與旅程總文。
- [x] 使用者可批次加入並混用直式限動截圖與原始照片。
- [x] 每個時刻可保存日期、選填時間、城市、地點、文案、歌名、歌手、選歌原因與選填 YouTube 連結。
- [x] 使用者可拖曳或透過按鈕調整時刻順序。
- [x] 草稿、自動儲存、重新載入與待補狀態正確運作。
- [x] 預覽不改變狀態、不自動播放。
- [x] 只有完成旅程會出現在 Atlas，且可透過既有流程回放。
- [x] `.soundpassport` 可完整匯出、清除後還原文字、照片、狀態與順序。
- [x] 匯入驗證與 transaction 可防止部分或損壞資料污染現有內容。
- [x] 私人資料不會寫入 Git 或送往應用程式後端。
